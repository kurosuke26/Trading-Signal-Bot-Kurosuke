...
# 【2026-09-16変更】90 → 75。酒田五法の全面改訂（sakata.py：定義どおりの検出＋実測精度）により、
# 旧版の乱発パターンに依存していた90点以上は発生しなくなった。先読みなしの検証
# （evaluate_strategy.py、前半で選び後半で確認）で、スコア閾値を上げるほど後半期間の
# 市場平均との差が単調に改善し（ATR×3.0：0点+0.53%→70点+1.05%→75点+1.41%/回）、
# 80点以上は件数不足だったため、75点を採用する。
...
# 【2026-09-16変更：LONG 5.0 → 3.0】上記の5.0倍は「シグナル日の終値でエントリー」「期待値そのもの」で
# 選んでいたが、先読みなし（翌営業日始値でエントリー・決済）かつ「同じ保有期間の市場平均との差」で
# 測り直すと、倍率を上げるほど保有が長くなり上昇相場に乗るだけの効果が大きくなっていた。
