...
## 業種内の相対評価（2026-09-16追加）

複合スコアだけの選定は市場平均を上回る一方、「同じ業種の平均」には届いていなかった（先読みなしの検証で、
後半期間：市場との差+1.4%／業種との差−0.5%）。そこで `sector_relative.py` が毎晩、同じ日・同じ業種の中での
位置を計算し、LONGの仮想エントリー条件と投稿表示に使う。

| 指標 | 計算 | 使い方 |
|---|---|---|
| 業種内モメンタム | その銘柄の60営業日リターン −（同業種の銘柄の60営業日リターンの平均） | **0以上を仮想エントリーの条件にする**（`LONG_REQUIRE_SECTOR_MOMENTUM=0` で無効化） |
| 業種内割安 | PER×PBR ÷ 同業種の中央値 | 表示のみ（条件にすると件数が減り、テーマ相場の期間はマイナスだった） |

条件を加えた効果（トレーリングATR×3.0・スコア75点以上）：業種との差が後半 −0.52% → **+0.33%**。
前半・後半・テーマ相場の期間（2025-10〜2026-05のMLCC相場）・期間外の4通りすべてでプラスになった。
同じ業種の対象が5銘柄未満・データ不足の場合は判定不能とし、条件では落とさない。
...
## 資金台帳・capital_tracker.py（2026-09-24追加）

「500万円を元手に、この日からbotのシグナル通り実際に運用していたら」を再現する資金台帳。
`collect_data.yml`でcollector.py実行後に呼ばれ、`data/trade_log.json`を日次で読んで
`data/capital_ledger.json`に資金の出入りを積み上げる。

- **対象は現物LONGのみ**（信用取引・SHORTは対象外。実際に空売りはしない）。
  上記のSHORT関連セクション（酒田五法の「売り」列や`#ショート-シグナル`投稿等）は
  あくまで情報提供・仮想検証であり、この資金台帳・実運用の対象ではない
- **SHORTシグナルの実運用上の位置づけは「警戒」**：保有中のLONG銘柄について、その日の
  判定がSHORTに変わった場合は`capital_tracker.py`が⚠️マーク付きで表示する
  （`apply_caution_flags`、`data/latest_scan.json`の当日判定を参照）。ポジションの
  トレーリングストップは変えず自動決済もしない、あくまで「弱気の目安が点灯した」という
  注意喚起であり、保有継続の可否は表示を見て自分で判断する
- 初期資金5,000,000円、1トレードあたりequityの10%（複利）、同時保有上限は投資済み合計が
  equityの80%まで（実質最大8ポジション）
- 台帳の開始日以降に新規エントリーしたポジションのみが対象（過去の一括ロード分は含めない）
- 年が変わったら500万円・保有0件にリセットし、年またぎ中の保有分は前年分アーカイブに
  「注記上の決済」として記録する（`data/capital_ledger_archive/<年>.json`）
- 決済判定・トレーリングストップの計算ロジックはtracking.py本番ロジック（LONGプライマリ）を
  そのまま使い、ここでは二重実装しない
