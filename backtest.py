...
- 株価はraw（分割未調整）のO/H/L/C/Voを使用する。期間中に株式分割があった銘柄は、
  分割前後で価格が不連続になりATR・移動平均が乱れる可能性がある点に注意。
  【2026-09-15追記】この「rawで統一」という前提が崩れた：本番のcollector.pyは
  yfinance取得をauto_adjust=True（分割・配当調整済み）に切り替えたため、現在は
  本番とこのバックテストで前提が食い違っている（本番＝調整済み、バックテスト＝
  raw）。J-Quantsクライアント（jquants_client.py）はAdjC（調整済み終値）フィールドを
  返す想定になっているが、実際のレスポンスで動作確認は取れていない。バックテストの
  数値を最新の本番ロジックの実証として引用する場合は、この食い違いを踏まえること
  （分割が起きていない銘柄への影響は軽微だが、分割銘柄はバックテスト側でまだ
  誤差が乗る可能性がある）。次にバックテストを再実行する機会があれば、AdjCへの
  切り替えも合わせて検討する。
